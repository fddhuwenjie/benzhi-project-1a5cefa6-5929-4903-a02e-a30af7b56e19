...
type readinessCacheEntry struct {
	revision int64
	value    qualification.ReviewReadiness
}

func New(s *store.Store) *Service {
	return &Service{store: s, readinessCache: make(map[string]readinessCacheEntry)}
}

func (s *Service) cachedReadiness(c *domain.ExerciseCase, reviewer string) qualification.ReviewReadiness {
	s.readinessMu.Lock()
	defer s.readinessMu.Unlock()
	if cached, ok := s.readinessCache[c.CaseID]; ok && cached.revision == c.Revision {
		return cached.value
	}
	value := qualification.BuildReviewReadiness(c, strings.TrimSpace(reviewer))
	s.readinessCache[c.CaseID] = readinessCacheEntry{revision: c.Revision, value: value}
	return value
}
...
func (s *Service) PrecheckProtocol(id string, cmd PrecheckProtocolCommand) (ProtocolPrecheckResult, error) {
	...
	return ProtocolPrecheckResult{CaseID: id, Revision: c.Revision, Status: c.Status, Check: check}, nil
}
...
func (s *Service) Review(id string, cmd ReviewCommand) (CommandResult, error) {
	out, err := s.execute(id, cmd.Meta, cmd, 200, func(snap *store.Snapshot) (CommandResult, error) {
		...
		readiness := s.cachedReadiness(c, cmd.ReviewerID)
...
func (s *Service) ReviewReadiness(id, reviewer string) (qualification.ReviewReadiness, error) {
	...
	return s.cachedReadiness(c, reviewer), nil
}
