...
func (Evaluator) Evaluate(p *domain.ProtocolBaseline, runs []domain.ExerciseRun) []domain.RuleResult {
	...
	cacheKey := domain.Digest(struct {
		Protocol *domain.ProtocolBaseline
		Runs     []domain.ExerciseRun
	}{Protocol: p, Runs: runs})
	evaluationCache.RLock()
	cached, ok := evaluationCache.results[cacheKey]
	evaluationCache.RUnlock()
	if ok {
		return cached
	}
	...
	evaluationCache.Lock()
	evaluationCache.results[cacheKey] = results
	evaluationCache.Unlock()
	return results
}
