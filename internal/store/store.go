...
func New(root string) (*Store, error) {
	...
	return &Store{root: root, snapshots: make(map[string]*snapshotCache)}, nil
}
func (s *Store) caseLock(id string) *sync.Mutex {
	return s.locks.forCase(id)
}
func (s *Store) cacheFor(id string) *snapshotCache {
	s.cacheMu.Lock()
...
func (s *Store) Load(id string) (*Snapshot, error) {
	...
	cached := s.cacheFor(id)
	cached.mu.RLock()
	if cached.value != nil {
		clone, err := cloneSnapshot(cached.value)
		cached.mu.RUnlock()
		return clone, err
	}
	cached.mu.RUnlock()
