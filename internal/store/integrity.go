...
func (s *Store) DiagnoseCertificate(id string) (CertificateDiagnostic, error) {
	...
	cached, ok := s.certificateDiagnostics[id]
	...
	if err := json.Unmarshal(body, &certificate); err != nil {
		...
		s.rememberCertificateDiagnostic(id, diagnostic)
...
func (s *Store) rememberCertificateDiagnostic(id string, diagnostic CertificateDiagnostic) {
	s.certificateCacheMu.Lock()
	s.certificateDiagnostics[id] = diagnostic
	s.certificateCacheMu.Unlock()
}
